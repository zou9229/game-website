...
function pemToPkcs8Bytes(privateKey: string) {
...
async function getAccessToken(serviceAccount: VertexServiceAccount) {
  ...
  if (!resp.ok) {
    ...
    throw new Error(
      `Vertex AI auth failed with status: ${resp.status}, body: ${errorText}`
    );
  }
...
export async function generateTextWithVertexGemini({
  ...
  if (!resp.ok) {
    const errorText = await resp.text();
    throw new Error(
      `Vertex AI request failed with status: ${resp.status}, body: ${errorText}`
    );
  }
